...
"""  # TODOn: 1. PUT YOUR NAME IN THE ABOVE LINE.
...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    # TODOn: 2. Implement this TEST function.
...
def run_test_pizza():
    ...
    title = 'PIZZA test 4:  4 slices, thin (thickness=3) red lines.'
    window = rg.RoseWindow(400, 400, title)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    circle.attach_to(window)
    points = draw_points_on_circle(window, circle, number_of_points_on_a_circle, color)
    lines = Line(points, points)
    for k in range(len(lines)):
        center = lines[k]
        circle = rg.Circle(center, 10)
        circle.fill_color = color
        lines.outline_thickness = thickness
        circle.attach_to(window)
        center.attach_to(window)
        window.render(0.05)
